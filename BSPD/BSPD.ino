...
unsigned long us_current = 0;
unsigned long us_at_last_tx = 0;
unsigned long channel_switch = false;
...
void setup() {
  ...
  // Give the voltages array initial values
  voltages[2] = "0.0";
  voltages[3] = "0.0";
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // instructions within
      ...
      if (dpot_pos_1 < 1 || dpot_pos_1 > 256) {
        ...
        i2c_write_success = (ch1.write(dpot_pos_1 - 1) == 0);
      ...
      if (dpot_pos_2 < 1 || dpot_pos_2 > 256) {
        ...
        i2c_write_success = (ch2.write(dpot_pos_2 - 1) == 0);
        if (!i2c_write_success) {
  ...
  // delayMicroseconds(150);
  us_current = micros();
  if (us_current - us_at_last_tx > 5000) {
    if (channel_switch) {
    ...
    us_at_last_tx = us_current;
  }
  voltages[4] = (digitalRead(DEBUG_CH_4) == HIGH ? "HI" : "LO");
  voltages[5] = voltages[4];
  voltages[1] = String(0);
  voltages[0] = String(0);
